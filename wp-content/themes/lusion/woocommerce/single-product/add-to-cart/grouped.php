	...
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table">
		<tbody>
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				$image_url       = get_the_post_thumbnail_url( $grouped_product_child->get_id(), array(60,80) );
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
						case 'image':
							$value  = '<img src="' . esc_url($image_url) . '" alt="' .  esc_attr( $product->get_name() ) . '"/>';
							break;
						case 'label':
							$value  = '<label for="product-' . esc_attr( $grouped_product_child->get_id() ) . '">';
							$value .= $grouped_product_child->is_visible() ? '<h4><a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', $grouped_product_child->get_permalink(), $grouped_product_child->get_id() ) ) . '">' . $grouped_product_child->get_name() . '</a></h4>' : $grouped_product_child->get_name();
							$value .= '</label>';
							break;
