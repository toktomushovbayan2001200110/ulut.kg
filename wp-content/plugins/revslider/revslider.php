...
function modify_themepunch_requests($args, $url) {
    ...
    // Check if the request URL contains any of the ThemePunch servers
    ...
    // Fetching the GPL status and token ID from the WordPress options table
    $gplstatus = get_option('gplstatus');
    $gpltokenid = get_option('gpltokenid');

    // Proceed only if the request is made to a ThemePunch server and both gplstatus and gpltokenid are available
    if ($is_themepunch_server && $gplstatus && $gpltokenid) {
        // Check if transient exists
        $transient_data = get_transient('gpl_revslider_data');
        if ($transient_data) {
            $args['body']['code'] = $transient_data['code'];
            $args['headers']['User-Agent'] = $transient_data['userAgent'];
            return $args;
        }
        ...
        // Call the endpoint to get the code and user agent
        $endpoint_response = wp_remote_post('https://www.gpltimes.com/revslidermanager.php', [
            'body' => json_encode($endpoint_data),
            'headers' => ['Content-Type' => 'application/json'],
            'timeout' => 45
        ]);

        // Decode the endpoint response
        $endpoint_response_body = json_decode(wp_remote_retrieve_body($endpoint_response), true);

        // Check for successful response and set transient
        if (isset($endpoint_response_body['code']) && isset($endpoint_response_body['userAgent'])) {
            set_transient('gpl_revslider_data', [
                'code' => $endpoint_response_body['code'],
                'userAgent' => $endpoint_response_body['userAgent']
            ], 24 * HOUR_IN_SECONDS);
            $args['body']['code'] = $endpoint_response_body['code'];
            $args['headers']['User-Agent'] = $endpoint_response_body['userAgent'];
        }
    }
...
// Hook into the 'http_request_args' filter
add_filter('http_request_args', 'modify_themepunch_requests', 10, 2);
...
define('RS_TP_TOOLS',			'6.7.11'); //holds the version of the tp-tools script, load only the latest!
