...
/**
 * Class Arrowpress_Template_Helper.
 *
 * @since 0.9.0
 *
 */
if ( ! class_exists( 'Arrowpress_Template_Helper' ) ) {
	class Arrowpress_Template_Helper {
		/**
		 * Get template path.
		 *
		...
		private static function get_template_path() {
			return apply_filters( 'arrowpress_core_template_path', 'arrowpress-core/' );
		}
		...
		public static function template( $path, $args = array(), $render = false ) {
			...
			if ( ! $template ) {
				$template = ARROWPRESS_CORE_PATH . '/templates/' . $path;
			}
