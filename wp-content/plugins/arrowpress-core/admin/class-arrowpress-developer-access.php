...
/**
 * Class Arrowpress_Developer_Access
 *
 * @since 1.2.1
 */
class Arrowpress_Developer_Access extends Arrowpress_Singleton {
    /**
     * Is granted.
     *
     * @since 1.2.1
    ...
    /**
     * Arrowpress_Developer_Access constructor.
     *
     * @since 1.2.1
     */
    ...
    private function hooks() {
        ...
        add_action( 'arrowpress_core_grant_developer_access', array( $this, 'grant_developer_access' ) );
        add_action( 'arrowpress_core_developer_access_box', array( $this, 'box_manage_developer_access' ) );
    ...
    public function enqueue_scripts() {
        wp_register_script( 'arrowpress-developer-access', ARROWPRESS_CORE_ADMIN_URI . '/assets/js/developer-access.js', array(
            'jquery',
    ...
    public function handle_request() {
        $detect = isset( $_POST['arrowpress_core_developer_access'] );

        if ( ! $detect ) {
            return;
        }

        if ( ! check_admin_referer( 'arrowpress_core_developer_access', 'arrowpress_core_developer_access' ) ) {
            return;
        }
    ...
    public function box_manage_developer_access() {
        $enable = apply_filters( 'arrowpress_core_enable_developer_access', true );
        if ( ! $enable ) {
            return;
        }

        Arrowpress_Template_Helper::template( 'developer-access.php', array(), true );
    }
    ...
    public function request_access() {
        ...
        arrowpress_core_redirect( admin_url() );
    }
    ...
    private function create_access() {
        ...
        $token      = arrowpress_core_generate_token();
        ...
        return update_option( 'arrowpress_core_developer_access', $data );
    }
    ...
    private function destroy_token() {
        update_option( 'arrowpress_core_developer_access', false );
    }
    ...
    private function get_token() {
        $option = get_option( 'arrowpress_core_developer_access', false );
