...
/**
 * Class Arrowpress_Widget_Importer_Service
 *
 * @since 0.4.0
 */

if ( ! class_exists( 'Arrowpress_Widget_Importer_Service' ) ) {
	class Arrowpress_Widget_Importer_Service {
		/**
		 * Remove all widgets
		 *
		...
		public function import( $json_data, $widget_logic, $clear_current = true ) {
			...
			do_action('arrowpress_core_after_import');
		}
		...
		private static function mapping_nav_menu() {
			...
			$terms          = $arrowpress_wp_import->processed_terms;
		...
		private static function mapping_layout_builder() {
			$widgets = get_option( 'widget_arrowpress_layout_builder' );
			...
			foreach ( $widgets as $key => $widget ) {
				...
				$content           = Arrowpress_Importer_Mapping::mapping_nav_menu_in_content_shortcode( $content );
				$widget['content'] = $content;
				$widgets[ $key ]   = $widget;
			}

			update_option( 'widget_arrowpress_layout_builder', $widgets );
		}
		...
		public static function map_so_settings_widget( $settings ) {
			...
			if ( is_array( $panels_info ) ) {
				...
				if ( $class === 'WP_Nav_Menu_Widget' ) {
					...
					$map_nav_id = Arrowpress_Importer_Mapping::get_map_nav_menu( $nav_id );
