...
require_once ARROWPRESS_CORE_ADMIN_PATH . '/includes/wordpress-importer/wordpress-importer.php';

/**
 * Class Arrowpress_WP_Import
 *
 * @since     0.3.1
 * @package   ArrowPress_Core_Admin
 */
class Arrowpress_WP_Import_Service extends WP_Import {
	/**
	 * @since 1.0.6
	 *
	 * @var null
	 ...
	 * @var string
	 */
	protected $_arrowpress_demo_content_key = '_arrowpress_demo_content';
	...
	/**
	 * Get single instance Arrowpress_WP_Import_Service.
	 *
	 * @since 1.0.6
	 *
	 * @param bool $clean
	 *
	 * @return Arrowpress_WP_Import_Service
	 */
	...
	/**
	 * Arrowpress_WP_Import constructor.
	 *
	 * @param bool $clean
	 */
	...
	private function allow_edit_variables() {
		$posts_pp       = get_option( 'arrowpress_importer_posts_pp' );
		$attachments_pp = get_option( 'arrowpress_importer_attachments_pp' );
		...
		if ( defined( 'ARROWPRESS_IMPORTER_POSTS_PP' ) ) {
			$this->_posts_per_page = ARROWPRESS_IMPORTER_POSTS_PP;
		}

		if ( defined( 'ARROWPRESS_IMPORTER_ATTACHMENTS_PP' ) ) {
			$this->_attachments_per_page = ARROWPRESS_IMPORTER_ATTACHMENTS_PP;
		}
	}
	...
	private function _get_demo_image() {
		...
		$image_path = apply_filters( 'arrowpress_core_importer_path_demo_image', $image_path );
		if ( file_exists( $image_path ) ) {
			$image['path'] = $image_path;
			$image['uri']  = apply_filters( 'arrowpress_core_importer_uri_demo_image', get_stylesheet_directory_uri() . '/inc/data/demo_image.jpg' );

			return $image;
		}

		return array(
			'path' => ARROWPRESS_CORE_ADMIN_PATH . '/assets/images/demo_image.png',
			'uri'  => ARROWPRESS_CORE_URI . '/admin/assets/images/demo_image.png',
		);
	}
	...
	private function _insert_attachment_demo_image() {
		...
		$args  = array(
			'meta_query'     => array(
				array(
					'key'   => '_arrowpress_demo_attachment',
					'value' => 'yes'
				)
			),
		...
		update_post_meta( $attach_id, '_arrowpress_demo_attachment', 'yes' );
	...
	public function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				do_action( 'arrowpress_import_processed_term', $id, $cat );
				...
				$log = ob_get_clean();
				arrowpress_add_log( $log );
				continue;
			}
			$this->process_termmeta( $cat, $id );
		}

		do_action( 'arrowpress_import_processed_terms', $this->processed_terms, $this->categories );
	...
	private function _import_posts( $posts, $attachment_demo_image_id = false ) {
		...
		foreach ( $this->posts as $post ) {
			do_action( 'arrowpress_import_import_post', $post );
		...
		if ( ! empty( $post['postmeta'] ) ) {
			foreach ( $post['postmeta'] as $meta ) {
				...
				if ( $key ) {
					...
					if ( ! $value ) {
						$value = arrowpress_maybe_unserialize( $meta['value'] );
					}
					...
					if ( $key === '_elementor_data' ) {
						$meta_handler = new Arrowpress_Elementor_Import_Service( $value, $this->_chunks['base_blog_url'] );
	...
	public function analyze_content( $xml ) {
		...
		$parser       = new WXR_Parser();
  		$default_file = Arrowpress_Importer::get_base_dir();

		if($default_file){
			$data_default = $parser->parse( $default_file .'content.xml' );
		}else{
			$data_default = array();
		}
		...
		if ( $data['posts'] ) {
			if ( $data_default['posts'] ) {
				$post_types_demo    = $this->get_data_file_content( $data );
				$post_types_default = $this->get_data_file_content( $data_default );
				$post_types         = array_merge_recursive( $post_types_default, $post_types_demo );
			} else {
				$post_types = $this->get_data_file_content( $data );
			}
		...
		foreach ( array( 'terms', 'categories', 'tags', 'authors' ) as $type ) {
			if ( ! empty( $data[$type] ) ) {
				if ( $data_default[$type] ) {
					$data_type[$type]  = array_merge_recursive(  $data[$type], $data_default[$type] );
					$this->_set_cache_data( $type, $data_type[$type] );
				}else{
		...
		$this->_chunks = $chunks;
		$this->_set_cache_data( 'chunks', $this->_chunks );
 		do_action( 'arrowpress_import_parse_demo_data', $this );
	}
	public function get_data_file_content($data){
		$posts      = $data['posts'];
	...
	public function get_current_demo( $field = '' ) {
		$current_demo = get_option( 'arrowpress_importer_current_demo' );
	...
	public function get_demo_cache_path() {
		...
		$this->_cache_dir = $upload_dir['basedir'] . "/arrowpress-import-demo/" . $this->get_current_demo( 'demo' );

		//$this->rmdir( $this->_cache_dir );

		if ( ! Arrowpress_File_Helper::mkdir( $this->_cache_dir ) ) {
			die( sprintf( __( 'Unable to create directory %s' ), $this->_cache_dir ) );
		}

		Arrowpress_File_Helper::mkdir( $this->_cache_dir . '/cache' );
	...
	public function import_object_meta( $return_me, $meta_key, $unused = '' ) {
		if ( '_arrowpress_demo_content' == $return_me ) {
			$return_me = false;
		}
	...
	private function _clean_taxes() {
		...
		$query = $wpdb->prepare( "
			...
			WHERE t.term_id <> %d
		", '_arrowpress_demo_content', $exclude );
	...
	private function _clean_comments() {
		...
		$query = $wpdb->prepare( "
			...
			WHERE cm.meta_key = %s
			AND cm.meta_value = %s
		", $this->_arrowpress_demo_content_key, 'yes' );
	...
	private function _clean_posts() {
		...
		// Firstly, we need to delete all posts (and it's meta data) has meta $this->_arrowpress_demo_content_key is 'yes'.
		...
		$query    = $wpdb->prepare( "
			...
			AND pm.meta_value = %s
			AND p.post_type <> %s
		", $this->_arrowpress_demo_content_key, 'yes', 'attachment' );
	...
	private function _clean_cache_folder() {
		Arrowpress_File_Helper::rmdir( $this->_cache_dir . '/cache' );
		Arrowpress_File_Helper::mkdir( $this->_cache_dir . '/cache' );
	}
	 ...
	 * @return mixed
	 */
	private function _set_object_is_demo_content( $object_type, $object_id, $key = '_arrowpress_demo_content', $value = 'yes' ) {
	...
	function fetch_remote_file( $url, $post ) {
		...
		$download = Arrowpress_Remote_Helper::download( $url, $upload['file'] );

		if ( is_wp_error( $download ) ) {
			arrowpress_add_log( $download->get_error_message(), $download->get_error_code() );
	...
	function backfill_parents() {
		...
		foreach ( $missing_menu_items as $item ) {
			if ( ! empty( $done_items[ $item['post_id'] ] ) ) {
				continue;
			}
			$this->process_menu_item( $item, true );
			$done_items[ $item['post_id'] ] = $item['post_id'];
		}
		...
		// Fixed for duplicating menu items
		$this->set_cache_objects();
