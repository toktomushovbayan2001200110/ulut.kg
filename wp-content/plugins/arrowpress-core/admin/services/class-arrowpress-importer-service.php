...
/**
 * Class Arrowpress_Importer_Service.
 *
...
class Arrowpress_Importer_Service {
	...
	private static function _file_get_contents( $file ) {
		...
		if ( ! $contents ) {
			throw Arrowpress_Error::create( 'Get file content failed!', 6, 'Please check file <code>' . $file . '</code>' );
		}
	...
	public static function theme_options( $options_file ) {
		try {
			...
			$theme_mods = arrowpress_clean_theme_mods( $theme_mods );

			// Update theme mods
			update_option( "theme_mods_$theme", $theme_mods );
		}
		catch ( Exception $e ) {
			throw Arrowpress_Error::create( $e->getMessage() );
		}
	...
	public static function change_theme_mods_root_url( &$item ) {
		$replace = new Arrowpress_Elementor_Import_Service( null, null );
	...
	public static function analyze_content( $xml ) {
		$arrowpress_wp_import = Arrowpress_WP_Import_Service::instance( true );
		$arrowpress_wp_import->analyze_content( $xml );
	}
	...
	public static function widget( $widget_file, $widget_logic ) {
		try {
			...
			if ( empty( $json_data ) || ! is_array( $json_data ) ) {
				throw Arrowpress_Error::create(
					'Import data could not be read.', 6,
					'Please check file <code>' . $widget_file . '</code>'
				);
			}

			$arrowpress_widget_importer = new Arrowpress_Widget_Importer_Service();
			$arrowpress_widget_importer->import( $json_data, $widget_logic, true );
		}
		catch ( Exception $e ) {
			throw Arrowpress_Error::create( $e->getMessage() );
		}
	...
	public static function revslider( $revsliders ) {
		try {
			...
			if ( $prefix_folder_data_demo ) {
				$dir_revsliders = Arrowpress_Admin_Config::get( 'demo_data' ) . esc_attr( $prefix_folder_data_demo ) . '/revsliders/';
			} else {
				$dir_revsliders = apply_filters( 'arrowpress_core_importer_directory_revsliders', $THEME_PATH . '/inc/data/revsliders/' );
			}
			...
			foreach ( $revsliders as $file ) {
				...
				$file = Arrowpress_File_Helper::download_file( $f );

				if ( is_wp_error( $file ) ) {
					throw Arrowpress_Error::create( 'Download slider data failed.', 8, '' );
				}
	...
	public static function settings( $settings_file ) {
		 ...
		 */
		$basic_settings = Arrowpress_Importer_Service::get_key_basic_settings();
		foreach ( $basic_settings as $basic_setting ) {
			if ( isset( $settings[$basic_setting] ) ) {
				// fix for theme modus
				if ( $basic_setting == 'modus_settings' ) {
					$modus_setting = Arrowpress_Importer_Mapping::mapping_nav_menu_in_theme_modus_setting( $settings['modus_settings'] );
					array_walk( $modus_setting, array( __CLASS__, 'change_theme_mods_root_url' ) );
 					update_option( 'modus_settings', $modus_setting );
				} else {
		 ...
		 */
		$settings_key = Arrowpress_Importer_Service::get_key_page_id_settings();
		foreach ( $settings_key as $key ) {

			$page_slug = isset( $settings[$key] ) ? $settings[$key] : false;

			$page_id = arrowpress_get_page_id_by_path( $page_slug );
	...
	public static function reset_data_demo() {
		...
		$arrowpress_wp_import->clean_demo_content();
		 ...
		 * Remove all widgets.
		 */
		Arrowpress_Widget_Importer_Service::remove_all();
		...
		arrowpress_clean_theme_mods( $theme_mods_backup );
		update_option( "theme_mods_$theme", $theme_mods_backup );

		Arrowpress_Importer::update_key_demo_installed( false );
	...
	public static function get_key_page_id_settings() {
		...
		return apply_filters( 'arrowpress_importer_page_id_settings', $list );
	}
...
//			'sb_instagram_settings',
		...
		return apply_filters( 'arrowpress_importer_basic_settings', $list );
	}
