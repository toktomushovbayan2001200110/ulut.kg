...
/**
 * Class Arrowpress_Envato_Service.
 *
 * @since 0.7.0
 */
class Arrowpress_Envato_API {
	...
	public static function get_url_download_item( $item_id ) {
		...
		return apply_filters( 'arrowpress_core_envato_api_get_url_download_item', $url, $item_id );
	}
	...
	public static function list_all_purchase_codes( $personal_token ) {
		...
		return apply_filters( 'arrowpress_core_envato_api_list_all_purchase_code', $purchase_codes );
	}
	...
	public static function get_theme_metadata( $id, $token ) {
		...
		return apply_filters( 'arrowpress_core_envato_api_get_theme_metadata', $data, $id );
	}
	...
	private static function request( $url, $token ) {
		if ( empty( $token ) ) {
			return new WP_Error( 'api_token_error', __( 'An API token is required.', 'arrowpress-core' ) );
		}
		...
		if ( 200 !== $response_code ) {
			if ( null === $return || empty( $return['error_description'] ) ) {
				return new WP_Error( 'api_error', __( 'An unknown API error occurred.', 'arrowpress-core' ) );
			}
