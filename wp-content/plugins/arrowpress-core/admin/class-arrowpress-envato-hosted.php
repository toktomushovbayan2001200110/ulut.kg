...
/**
 * Class Arrowpress_Envato_Hosted
 *
 * @since 1.6.0
 */
class Arrowpress_Envato_Hosted extends Arrowpress_Singleton {
	/**
	 * @since 1.6.0
	 *
	 * @var string
	...
	public static function get_url_verify_subscription_code() {
		$base_url = Arrowpress_Admin_Config::get( 'host_envato_app' ) . '/envato-hosted-subscription-code/';
	...
	/**
	 * Arrowpress_Envato_Hosted constructor.
	 *
	 * @since 1.6.0
	 */
	...
	private function _includes() {
		include_once ARROWPRESS_CORE_ADMIN_PATH . '/includes/tgmpa/class-tgm-plugin-activation.php';
	}
	...
	private function hooks() {
		add_filter( 'arrowpress_core_path_template_getting_started_updates', array(
			$this,
			'override_getting_started_step_updates'
		) );

		add_filter( 'arrowpress_core_list_plugins_required', array( $this, 'add_envato_market_plugin' ), 100 );
		add_filter( 'tgmpa_register', array( $this, 'register_tgmpa_plugins' ) );
		add_filter( 'arrowpress_core_production_registration_is_active', array( $this, 'filter_envato_hosted_active' ) );
		add_filter( 'arrowpress_core_get_link_download_plugin', array( $this, 'filter_link_download_premium_plugins' ) );
		add_filter( 'arrowpress_core_dashboard_box_classes', array( $this, 'add_classes_to_box_update' ) );
	}
	...
	public function register_tgmpa_plugins() {
		...
		foreach ( $all_plugins as $plugin ) {
			...
			$source   = null;
			if ( ! $is_wporg ) {
				$source = Arrowpress_Plugins_Manager::get_link_download_plugin( $slug );
			}
