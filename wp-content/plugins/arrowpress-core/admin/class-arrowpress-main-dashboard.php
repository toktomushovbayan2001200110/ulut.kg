 ...
 * @since 0.8.5
 */
class Arrowpress_Main_Dashboard extends Arrowpress_Admin_Sub_Page {
    /**
     * @var string
     *
     * @since 0.8.5
    ...
    public static function all_boxes() {
        if ( self::$boxes === null ) {
            ...
            self::$boxes = array(
                'appearance'    => array(
                    'id'    => 'appearance',
                    'title' => __( 'Appearance', 'arrowpress-core' ),
                ),
                'documentation' => array(
                    'id'    => 'documentation',
                    'title' => __( 'Help & Support', 'arrowpress-core' ),
                )
            );
            ...
            if ( $integrations_file ) {
                self::$boxes['integrations'] = array(
                    'id'    => 'integrations',
                    'title' => $theme_data['name'] .' '. __( 'Integrations', 'arrowpress-core' ),
                );
            }
            ...
            if ( $changelog_file ) {
                self::$boxes['changelog'] = array(
                    'id'    => 'changelog',
                    'title' => __( 'Changelog', 'arrowpress-core' ),
                );
            }
        }

        return apply_filters( 'arrowpress_dashboard_all_boxes', self::$boxes );
    }
    ...
    private function init_hooks() {
        add_action( 'wp_ajax_arrowpress_dashboard_order_boxes', array( $this, 'handle_order_boxes_dashboard' ) );
        add_filter( 'arrowpress_dashboard_sub_pages', array( $this, 'add_sub_page' ) );
        add_action( 'arrowpress_dashboard_registration_box', array( $this, 'render_registration_box' ), 10 );
        add_action( 'arrowpress_dashboard_boxes_left', array( $this, 'render_boxes_left' ) );
        add_action( 'arrowpress_dashboard_boxes_right', array( $this, 'render_boxes_right' ) );
    ...
    public function render_registration_box() {
        if ( Arrowpress_Product_Registration::is_active() ) {
            return;
        }
        ...
        Arrowpress_Dashboard::get_template( 'partials/registration.php' );
    }
    ...
    public function handle_order_boxes_dashboard() {
        ...
        update_option( 'arrowpress_dashboard_order_boxes', $post_data );
    ...
    private function render_box( $key ) {
        ...
        Arrowpress_Dashboard::get_template( 'boxes/master.php', $args );
    ...
    public function render_boxes_left() {
        ...
        $order = (array) get_option( 'arrowpress_dashboard_order_boxes', array() );
    ...
    public function add_sub_page( $sub_pages ) {
        $sub_pages['dashboard'] = array(
            'title' => __( 'Dashboard', 'arrowpress-core' ),
        );
