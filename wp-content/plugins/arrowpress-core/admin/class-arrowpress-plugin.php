...
if ( ! class_exists( 'Arrowpress_Plugin' ) ) {
	class Arrowpress_Plugin {
		...
		/**
		 * Get plugin file. Ex: arrowpress-core/arrowpress-core.php
		 *
		 * @return string
		 * @since 0.4.0
		 *
		...
		public function get_source() {
			...
			return apply_filters( 'arrowpress_core_url_download_private_plugin', $source, $this );
		}
		...
		public function get_text_status() {
			$arrText = array(
				'active'        => __( 'Active', 'arrowpress-core' ),
				'inactive'      => __( 'Inactive', 'arrowpress-core' ),
				'not_installed' => __( 'Not Installed', 'arrowpress-core' ),
			);
		...
		public function activate( $silent = null, $network_wide = false, $plugin_file_relative = '' ) {
			...
			if ( isset( $this->addon_dependency[ $plugin ] ) ) {
				foreach ( $this->addon_dependency[ $plugin ] as $addon_slug => $addon_label ) {
					if ( ! is_plugin_active( $addon_slug ) ) {
						return new WP_Error(
							'arrowpress_core_plugin_activate',
			...
			do_action( 'arrowpress_core_activate_plugin', $plugin, $network_wide, $silent );
		...
		public function install_by_local_file() {
			$file_path = $this->download_from_arrowpress();
		...
		public function wp_install( $package ) {
			do_action( 'arrowpress_core_pre_install_plugin', $package, $this );
		...
		public function update() {
			try {
				...
				if ( ! $this->is_wporg ) {
					$plugin = $this->download_from_arrowpress();
				...
				return new WP_Error( 'arrowpress_core_update_plugin', $e->getMessage() );
			}
		}
		 ...
		 */
		public function download_from_arrowpress() {
			...
			if ( ! empty( $data_json['message'] ) ) {
				return new WP_Error( 'arrowpress_err_download_plugin', ! empty( $data_json['message'] ) ? $data_json['message'] : __( 'Error download plugin', 'arrowpress-core' ) );
			}
