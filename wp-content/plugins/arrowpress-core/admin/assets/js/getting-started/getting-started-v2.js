...
(function ($, Arrowpress_YT, Arrowpress_Plugins_Queue) {
	...
	var Router = Backbone.Router.extend({
		...
		step: function (step) {
			emitEvent('arrowpress_gs_change_step', step);
		},
	...
	var Arrowpress_GS_Model = Backbone.Model.extend({
		initialize: function () {
			console.log("Model Arrowpress_GS_Model Initialize!");
		},
		defaults  : {
			isRunning: false
		}
	...
	var Arrowpress_Getting_Started = Backbone.View.extend({
		el: '.arrowpress-getting-started',

		router: false,

		model: null,

		base_url_ajax: arrowpress_gs.url_ajax,

		steps: arrowpress_gs.steps,
		...
		initialize: function () {
			 ...
			 */
			$(window).on('arrowpress_gs_change_step', function (e, args) {
				self.goToStep(args);
			});

			$(window).on('arrowpress_gs_complete', function () {
			...
			this.model = new Arrowpress_GS_Model();
			this.model.on('change:isRunning', this.toggleButtonSkip.bind(this));

			this.router = new Router();
			this.render();

			Arrowpress_Plugins_Queue.complete(function (data) {
				var action = data.action;
				var slug = data.slug;
				var $plugin = self.$('.arrowpress-table-plugins tr[data-plugin="' + slug + '"]');

				if (action === 'activate') {
					$plugin.removeClass('processing');
				}

				var remain = Arrowpress_Plugins_Queue.count();
				if (remain === 1) {
					self.$('.arrowpress-table-plugins').removeClass('running');
					self.model.set('isRunning', false);
					self.nextStep();
				}
			});

			Arrowpress_Plugins_Queue.success(function (response, data) {
				var action = data.action;
				if (action !== 'activate') {
					return;
				}
				...
				if (!response || !response.success) {
					...
					$plugin.removeClass('inactive processing').addClass('active').find('.arrowpress-input').prop('checked', false);
					$plugin.find('.updating-message').text('Active');
				}
			});

			Arrowpress_Plugins_Queue.error(function (data) {
				var action = data.action;
		...
		renderVideos: function () {
			...
			this.$('.arrowpress-video-youtube')
				.each(function () {
					var $video = $(this);
					var videoId = $video.attr('data-video');
					var player = Arrowpress_YT.createPlayer({
						id: videoId
					});
		...
		goToStep: function (step) {
			...
			if (this.steps.length - 1 === this.current_step) {
				emitEvent('arrowpress_gs_complete');
			}
		...
		_request_install_plugins: function () {
			...
			this.$('.arrowpress-table-plugins').addClass('running');
			var $plugins = this.$('.arrowpress-plugins input.arrowpress-input:checked');

			if (!$plugins || !$plugins.length) {
				this.model.set('isRunning', false);
				this.$('.arrowpress-table-plugins').removeClass('running');
				self.nextStep();
				return;
			}
			...
			$plugins.each(function () {
				...
				self.$('.arrowpress-table-plugins tr[data-plugin="' + slug + '"]').addClass('processing');
				if (status === 'not_installed') {
					Arrowpress_Plugins_Queue.push({action: 'install', slug: slug});
				}

				Arrowpress_Plugins_Queue.push({action: 'activate', slug: slug});
			});
		}
	});
	...
	$(document).ready(function () {
		new Arrowpress_Getting_Started();
	});
})(jQuery, Arrowpress_Video_Youtube, Arrowpress_Plugins_Queue);
