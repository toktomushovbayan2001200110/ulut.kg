(function ($) {
    const ArrowThemeMetaboxTabs = () => {
        $( document.body ).on( 'arrowpress-metabox-tab', function() {
            $( 'ul.arrowpress-metabox-tab__title' ).show();

            $( 'ul.arrowpress-metabox-tab__title a' ).on( 'click', function( e ) {
                e.preventDefault();

                const panelWrap = $( this ).closest( 'div.arrowpress-metabox-tab' );

                $( 'ul.arrowpress-metabox-tab__title li', panelWrap ).removeClass( 'active' );

                $( this ).parent().addClass( 'active' );

                $( 'div.arrowpress-metabox-tab__content--inner', panelWrap ).hide();

                $( 'div.arrowpress-metabox-tab__content--inner[data-content="' + $( this ).data( 'content' ) + '"]', panelWrap ).show();
            } );

            $( 'div.arrowpress-metabox-tab' ).each( function() {
                $( this ).find( 'ul.arrowpress-metabox-tab__title li' ).eq( 0 ).find( 'a' ).trigger( 'click' );
            } );
        } ).trigger( 'arrowpress-metabox-tab' );
    };

    const thimMetaboxFileInput = () => {
        $( '.arrowpress-meta-box__file' ).each( ( i, element ) => {
            let thimImageFrame;

            const imageGalleryIds = $( element ).find( '.arrowpress-meta-box__file_input' );
            const listImages = $( element ).find( '.arrowpress-meta-box__file_list' );
            const btnUpload = $( element ).find( '.btn-upload' );
            const isMultil = !! $( element ).data( 'multil' );
            const isImg_url = !! $( element ).data( 'img_url' );
                    ...
                    selection.forEach( function( attachment ) {
                        ...
                        if ( attachment.id ) {
                            ...
                            if ( attachment.type === 'image' ) {
                                ...
                                listImages.append(
                                    '<li class="arrowpress-meta-box__file_list-item image" data-attachment_id="' + attachment.id + '"><img src="' + attachmentImage +
                                    '" /><ul class="actions"><li><a href="#" class="delete"></a></li></ul></li>'
                                );
                            } else {
                                listImages.append(
                                    '<li class="arrowpress-meta-box__file_list-item image" data-attachment_id="' + attachment.id + '"><img class="is_file" src="' + attachment.icon +
            ...
            if ( isMultil ) {
                listImages.sortable( {
                    ...
                    helper: 'clone',
                    opacity: 0.65,
                    placeholder: 'arrowpress-metabox-sortable-placeholder',
                    start( event, ui ) {
    ...
    const thimSelect2 = () => {
        if ( $.fn.select2 ) {
            $( '.arrowpress-select-2 select' ).select2();
        }
    };

    const thimMetaboxColorPicker = () => {
        $( '.arrowpress-meta-box__color--input' ).wpColorPicker();
    };

    const thimShowHide = () => {
        const metaBoxes = document.querySelectorAll( '.arrowpress-meta-box' );
                            ...
                            switch( type ) {
                                case 'checkbox':
                                    if ( dataHide[1] == '!=' && dataHide[2] !== Boolean( eleHide.checked ) ) {
                                        ...
                                        if ( field.classList.contains('arrowpress-meta-box-group') ) {
    ...
    $(document).ready( function() {
        ...
        function activeTab(obj)  {
            $('.metabox-tabs-container ul li').removeClass('active');
            $(obj).addClass('active');
            var id = $(obj).find('a').attr('href');
            $('.metabox-content > li').hide();
            $(id) .show();
        }

        $('.metabox-tabs li').on('click', function(){
            activeTab(this);
            return false;
        });
        activeTab($('.metabox-tabs li:first-child'));
    } );
})(jQuery);
...
jQuery( function ( $ ) {
    ...
    function init() {
        $( '.arrowpress-meta-box' ).each( function () {
            var $this = $( this ),
