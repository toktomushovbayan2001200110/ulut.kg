( function( $ ) {
  function _parseJSON( data ) {
    var m = data.match(
        /<!-- ARROWPRESS_IMPORT_START -->(.*)<!-- ARROWPRESS_IMPORT_END -->/ );
    try {
      if (m) {
        data = $.parseJSON( m[1] );
      } else {
  ...
  var Importer = Backbone.Model.extend( {
    initialize: function() {
      ...
      plugins_required: [], demo: null, packages: [],
    },
  } );

  var Arrowpress_Importer = Backbone.Model.extend( {
    initialize: function() {
    }, defaults: {
      demos: [],
    },
  ...
  var Arrowpress_Form_Import = Backbone.View.extend( {
    el: '.tc-modal-importer', doing_import: false, current_request: null,
     ...
     */
    initialize: function() {
      this.model = new Importer( arrowpress_importer_data );
      this.render();

      var self = this;
      $( document ).
          on( 'click', '.tc-modal-importer .tc-open-modal', function( e ) {
            self.close();
          } );

      // $( window ).on( 'arrowpress_core_open_modal', function( e, data ) {
      //   if ('tc-send-feedback' !== data) {
      //     return;
      //   }
      //
      //   $( document ).
      //       find( '#tc-send-feedback .content textarea' ).
      //       text( self.message_error );
      //   $( document ).
      //       find( '#tc-send-feedback .developer-access input' ).
      //       trigger( 'click' );
      //   self.close();
      // } );
    },
    ...
    open: function( demo ) {
      ...
      var $arrowpress_dashboard = $( '.arrowpress-dashboard' );
      $arrowpress_dashboard.addClass( 'arrowpress-modal-open' );
    ...
    close: function() {
      if (this.doing_import) {
        var r = window.confirm( arrowpress_importer_data.confirm_close );
      ...
      var $arrowpress_dashboard = $( '.arrowpress-dashboard' );
      $arrowpress_dashboard.removeClass( 'arrowpress-modal-open' );
    ...
    _initImport: function() {
      ...
      let params = {
        action: 'arrowpress_importer',
        demo: demo.key,
        packages: packages,
        retry: retry,
        step: 'start', //totalStep: packages.length
      };

      self._handleStep( params );
    },

    _handleStep: function( param ) {
      const self = this;
      const url_ajax = this.model.get( 'url_ajax' );
      const elLiPackageCurrent = $( 'li[data-package=' + param.step + ']' );

      elLiPackageCurrent.attr( 'data-status', 'running' );

      this._scrollTo( param.step );

      this.current_request = $.ajax( {
        url: url_ajax, method: 'POST', data: param, dataType: 'json',
      } ).success( function( res ) {
        if ('success' === res.status) {
          const elPackageProgressBar = elLiPackageCurrent.find(
              '.package-progress-bar' );

          if (elPackageProgressBar) {
            elPackageProgressBar.css( 'width', res.percentage + '%' );
          }

          if ('' !== res.step) {
            if (res.step !== param.step) {
              elLiPackageCurrent.attr( 'data-status', 'completed' );
            }

            param.step = res.step;
            param.extra = res.extra;

            setTimeout( function() {
              self._handleStep( param );
            }, 500 ); // Set larger latency with some servers, too fast consecutive call request will get 404
          } else {
            self._notifySuccess();
          }
        } else {
          elLiPackageCurrent.removeAttr( 'data-status' );
          self._notifyError( res.message );
        }
      } ).error( function( error ) {
        if (self.retry < self.retry_max) {
          return self._handleStep( param );
        }
    ...
    _stepByStep: function( step ) {
      ...
      this.current_request = $.ajax( {
        url: this.model.get( 'url_ajax' ), method: 'POST', dataType: 'text',//'json'
        data: {
          action: 'arrowpress_importer', time: ( new Date() ).getTime(),
        },
        ...
        if (! response) {
          ...
          if (self.try_again_step > self.try_again_step_max) {
            return self._notifyError( {
              code: '#002', title: 'Something went wrong!',
            } );
          }
        ...
        if (! success) {
          ...
          var code = data.code || '';
          if (self.try_again_step > self.try_again_step_max ||
              code.includes( '008_DOWNLOAD_FAILED' )) {
        ...
        var $done_wrap;
        if (done_step) {
          $done_wrap = self.$( '.package.' + done_step ).
              attr( 'data-status', 'completed' );
          $done_wrap.attr( {'data-percentage': '100%'} ).
              find( '.package-progress-bar' ).
              css( 'width', '100%' );
        } else {
          if (data.ext) {
            if ($.inArray( data.next, ['media', 'main_content', 'plugins'] ) !==
                -1) {
              var percentage = data.ext.percentage || 2;
              $done_wrap = self.$( '.package.' + data.next ).
                  attr( {'data-percentage': percentage + '%'} );
              $done_wrap.find( '.package-progress-bar' ).
    ...
    _notifyErrorAjax: function( error ) {
      var l18n = arrowpress_importer_data.details_error;
    ...
    _notifyError: function( message ) {
      this._emitEvent( 'arrowpress_importer_failed' );
      const $detail_error = this.$( '.wrapper-finish .details-error' );
      //$detail_error.find( '.get-support .error-code' ).text( details.code );
      $detail_error.find( 'h3' ).html( message );

      this.$( '.wrapper-finish' ).
          removeClass( 'success retry' ).
          addClass( 'failed' );
      this._finish();

      this.message_error = '[Import Demo Content] ' + message;
    ...
    _notifySuccess: function() {
      this.failed = false;
      this._emitEvent( 'arrowpress_importer_complete', this.model.get( 'demo' ) );
     ...
     * @since 0.8.6
     *
     * @returns {Arrowpress_Form_Import}
     */
    render: function() {
  ...
  var Arrowpress_Form_Uninstall = Backbone.View.extend( {
    ...
    events: {
      'click .close': 'close', 'click .tc-start': 'startUninstall',
    },
    ...
    startUninstall: function() {
      ...
      this.current_request = $.ajax( {
        ...
        dataType: 'text',
        data: {
          action: 'arrowpress_importer_uninstall',
        },
      } ).success( function( response ) {
        response = _parseJSON( response );

        if (response.success) {
          self.notify( 'success', arrowpress_importer_data.uninstall_successful );
          emitEvent( 'arrowpress_uninstall_demo_successful' );
        } else {
          self.notify( 'error', arrowpress_importer_data.uninstall_failed );
          emitEvent( 'arrowpress_uninstall_demo_failed' );
        }
      } ).error( function( error ) {
        ...
        self.notify( 'error', arrowpress_importer_data.something_went_wrong );
        emitEvent( 'arrowpress_uninstall_demo_failed' );
      } ).complete( function() {
        self.$( '.tc-start' ).removeClass( 'updating-message' );
        self.$el.removeClass( 'running' );
        emitEvent( 'arrowpress_uninstall_demo_complete' );
      } );
    },
  ...
  var Arrowpress_Importer_V2 = Backbone.View.extend( {
    el: '.tc-importer-wrapper', form_install: null, form_uninstall: null,
    ...
    events: {
      ...
      'click .arrowpress-screenshot': 'openInstall',
      'click .btn-uninstall': 'openUninstall',
    },

    initialize: function() {
      this.audio_complete = new Audio(
          'https://thimpresswp.github.io/thim-core/static/complete.mp3' );

      $( window ).
          on( 'arrowpress_importer_complete', this.onImportSuccess.bind( this ) );
      $( window ).
          on( 'arrowpress_uninstall_demo_successful',
              this.onUninstallSuccess.bind( this ) );
      this.model.on( 'change:installed',
          this.updateDemoInstalled.bind( this ) );
      this.render();
      this.form_install = new Arrowpress_Form_Import();
      this.form_uninstall = new Arrowpress_Form_Uninstall();
    },
    ...
    updateDemoInstalled: function() {
      ...
      this.$( '.arrowpress-demo' ).removeClass( 'installed active' );
    ...
    openInstall: function( e ) {
      ...
      if (!ArrowPress_Core.check_active()) {
          return;
      }

      var target = e.target;
      var $arrowpress_demo = $( target ).closest( '.arrowpress-demo' );
      var demo_key = $arrowpress_demo.data( 'arrowpress-demo' );
  ...
  $( document ).ready( function() {
    new Arrowpress_Importer_V2( {
      model: new Arrowpress_Importer( arrowpress_importer ),
    } );
  } );
} )( jQuery );
