 ...
 * @since 0.4.0
 */
(function ($, Arrowpress_Plugins, Arrowpress_Plugins_Queue, wp, ArrowPress_Core) {
	$(document).ready(function () {
		arrowpress_plugins.init();
	});
	...
	var arrowpress_plugins = {
		...
		count: function () {
			...
			var $notification_count = $('#arrowpress-core-count-notification');
		...
		filter_search: function () {
			var $wrapper = $('.arrowpress-wrapper .plugin-tab');
		...
		onEvent: function () {
			...
			Arrowpress_Plugins_Queue.success(
			...
			Arrowpress_Plugins_Queue.error(function (data, error) {
				var slug = data.slug;
			...
			$(document).on('click', '.plugin-action-buttons button:not(.updating-message)', function (e) {
				...
				var premium = 0;
				if($self.attr('data-premium') !== undefined ){
					premium = $self.attr('data-premium');
				}
 				if ((action === 'update' || action === 'install') && ( premium ==='1') ) {
				...
				Arrowpress_Plugins_Queue.push({action: action, slug: slug});
			});
		}
	}

})(jQuery, Arrowpress_Plugins, Arrowpress_Plugins_Queue, window.wp, window.ArrowPress_Core);
