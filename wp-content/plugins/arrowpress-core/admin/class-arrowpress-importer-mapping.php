...
/**
 * Class Arrowpress_Importer_Mapping
 *
 * @since 1.0.1
 */
class Arrowpress_Importer_Mapping extends Arrowpress_Singleton {
	/**
	 * @since 1.0.1
	 *
	 * @var null
	...
	public static function get_terms_mapping() {
		if ( self::$terms === null ) {
			...
			self::$terms    = $arrowpress_wp_import->processed_terms;
		}
	...
	/**
	 * Arrowpress_Importer_Mapping constructor.
	 *
	 * @since 1.0.1
	 */
	...
	private function mapping_content_so_builder( $value ) {
		...
		foreach ( $child_widgets as $i => $child_widget ) {
			$new_widget = Arrowpress_Widget_Importer_Service::map_so_settings_widget( $child_widget );
	...
	public static function mapping_nav_menu_in_theme_modus_setting( $content ) {
		$theme_options = maybe_unserialize( $content );
		$options      = array();
		foreach ( $theme_options as $key => $val ) {
			$options[$key] = $val;

			for ( $i = 1; $i <= 12; $i ++ ) {
				if ( $key == 'header_' . $i . '_menu' ) {
					$nav_id     = isset( $val ) ? $val : false;
					$map_nav_id = self::get_map_nav_menu( $nav_id );
  					if ( $map_nav_id ) {
						$options["header_{$i}_menu"] = $map_nav_id;
					}
				}
			}
