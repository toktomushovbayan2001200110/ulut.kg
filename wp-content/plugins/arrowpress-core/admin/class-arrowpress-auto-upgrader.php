...
/**
 * Class Arrowpress_Theme_Upgrader.
 *
 * @since 0.9.0
 */
class Arrowpress_Auto_Upgrader extends Arrowpress_Singleton {
	/**
	 * Arrowpress_Theme_Upgrader constructor.
	 *
	 * @since 0.9.0
	 */
	...
	private function init_hooks() {
		...
		add_action( 'arrowpress_core_check_update_external_plugins', array( $this, 'check_update_external_plugins' ) );
	}
	...
	public function check_update_external_plugins() {
		$data           = get_transient( 'arrowpress_core_update_plugins' );
		$update_plugins = is_array( $data ) ? $data : array();

		$api_check_update_plugin = Arrowpress_Admin_Config::get( 'api_update_plugins' );
		$plugin_checker          = new Arrowpress_Plugin_Upgrader( $api_check_update_plugin );

		$plugins = Arrowpress_Plugins_Manager::get_external_plugins();
		...
		set_transient( 'arrowpress_core_update_plugins', $update_plugins );
	}
	...
	public function pre_update_plugin( $options ) {
		...
		foreach ( $plugins as $plugin ) {
			if ( $plugin_file === $plugin->get_plugin_file() ) {
				$options['package'] = Arrowpress_Plugins_Manager::get_link_download_plugin( $plugin->get_slug() );
	...
	public function pre_update_theme( $options ) {
		...
		$return = apply_filters( 'arrowpress_core_get_link_download_theme', false, $theme );
		if ( is_string( $return ) ) {
		...
		$stylesheet = Arrowpress_Theme_Manager::get_current_theme();
		if ( $theme == $stylesheet ) {
			$url_download = Arrowpress_Product_Registration::get_url_download_theme( $stylesheet );
			if ( ! is_wp_error( $url_download ) ) {
				$options['package'] = $url_download;
			} else {
				if ( $url_download->get_error_code() == 'arrowpress_core_key_broken' ) {
					Arrowpress_Product_Registration::destroy_active();
				}
			}
	...
	public function inject_update_plugins( $value ) {
		...
		if ( $detect_ajax_update !== 'arrowpress_plugins_manager' ) {
			return $value;
		}
		...
		$all_plugins      = Arrowpress_Plugins_Manager::get_plugins();
		$external_plugins = get_transient( 'arrowpress_core_update_plugins' );
	...
	public function inject_update_themes( $value ) {
		...
		if ( $detect_ajax_update !== 'arrowpress_core_update_theme' ) {
			return $value;
		}

		Arrowpress_Product_Registration::double_check_theme_update();

		$theme_data         = Arrowpress_Theme_Manager::get_metadata();
		$template           = $theme_data['template'];
		$update_themes      = Arrowpress_Product_Registration::get_update_themes();
		$update_free_themes = Arrowpress_Free_Theme::get_update_themes();
	...
	public function exclude_check_update_themes_from_wp_org( $request, $url ) {
		...
		$slug = Arrowpress_Theme_Manager::get_current_theme();
	...
	private function get_exclude_plugins_update() {
		...
		return apply_filters( 'arrowpress_core_exclude_plugins_check_update', $exclude_plugins );
	}
}