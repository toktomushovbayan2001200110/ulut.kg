...
/**
 * Class Arrowpress_Importer_AJAX
 *
 * @package   ArrowPress_Core_Admin
 * @since     0.3.1
 * @version   1.10.1
 */
class Arrowpress_Importer_AJAX {
	 ...
	 * @since 0.6.0
	 */
	private static $key_option_current_demo = 'arrowpress_importer_current_demo';
	...
	public function handle_ajax_uninstall() {
		Arrowpress_Importer_Service::reset_data_demo();
	...
	public function handle_ajax() {
		$res = new Arrowpress_Import_REST_Response();

		try {
			if ( ! isset( $_POST['packages'] ) || count( $_POST['packages'] ) == 0 ) {
				throw Arrowpress_Error::create( __( 'Invalid packages!', 'arrowpress-core' ), 10 );
			}

			if ( ! isset( $_POST['step'] ) ) {
				throw Arrowpress_Error::create( __( 'Invalid step!', 'arrowpress-core' ), 10 );
			}
			...
			if ( 'start' == $step ) {
				if ( ! isset( $_POST['demo'] ) ) {
					throw Arrowpress_Error::create( __( 'Invalid demo!', 'arrowpress-core' ), 10 );
				}

				$demo  = sanitize_text_field( $_POST['demo'] );
				$demos = Arrowpress_Importer::get_demo_data( true );

				if ( ! isset( $demos[$demo] ) ) {
					throw Arrowpress_Error::create( __( 'Demo not found!', 'arrowpress-core' ) );
				}
				...
				if ( $_initializeImporter ) {
					$res->status  = 'success';
					$res->step    = $packages[0];
					$res->message = esc_html__( 'Prepare Importer success', 'arrowpress-core' );
				}
			} else {
				/**
				 * @var $res_step Arrowpress_Import_REST_Response
				 */
				$res_step = $this->_step_by_step( $step );

				//do_action( 'arrowpress_core_importer_next_step', $done, $next, $ext );
	...
	private function check_retry_import() {
		...
		$current_posts_pp       = get_option( 'arrowpress_importer_posts_pp', 100 );
		$current_attachments_pp = get_option( 'arrowpress_importer_attachments_pp', 10 );

		update_option( 'arrowpress_importer_posts_pp', max( intval( $current_posts_pp / 2 ), 10 ) );
		update_option( 'arrowpress_importer_attachments_pp', max( intval( $current_attachments_pp / 2 ), 1 ) );
	}
	...
	private function _initializeImporter( $demo, $packages = array() ) {
		do_action( 'arrowpress_core_importer_start_import_demo', $demo );

		$packages = (array) $packages;
		$packages = apply_filters( 'arrowpress_core_importer_prepare_packages', $packages );
		$this->_save_current_demo_data( $demo, $packages );
		update_option( 'arrowpress_importer_prepare_wp_import', false );
	...
	private function _prepare_demo_content() {
		...
		Arrowpress_Importer_Service::analyze_content( $xml);
	}
	...
	private function _save_current_demo_data( $demo, $packages ) {
		...
		return update_option(
			...
			array(
				...
				'packages'               => $packages,
				'default_content'        => isset($demo['default_content']) ? $demo['default_content'] : true,
				'dir'                    => $demo['dir'],
				'plugins_required'       => isset( $demo['plugins_required'] ) ? $demo['plugins_required'] : false,
				'default_xml'            => isset( $demo['default_xml'] ) ? $demo['default_xml'] : false,
	...
	private function _get_dir_current_demo() {
		...
		$base_dir = apply_filters( 'arrowpress_core_importer_base_path_demo_data', false, $current_demo );
		if ( ! $base_dir ) {
			return $current_demo['dir'];
		}

		return apply_filters(
			'arrowpress_core_importer_directory_current_demo',
	...
	private function step_finish() {
		 ...
		 */
		$delete_hello_post = get_page_by_path( 'hello-world', OBJECT, 'post'  );
		 ...
		 */
		$arrowpress_wp_import = Arrowpress_WP_Import_Service::instance( false );
		$arrowpress_wp_import->set_menu_locations();
		...
		Arrowpress_Importer::update_key_demo_installed( $demo_key );
		$this->import_end();
	}
...
//		if ( class_exists( '\Arrowpress_EL_Kit\Modules\Cache' ) ) {
//			\Arrowpress_EL_Kit\Modules\Cache::instance()->regenerate();
//		}
	}
	...
	private function _update_settings() {
		...
		Arrowpress_Importer_Service::settings( $settings_file );
	}
	 ...
	 *
	 * @return mixed
	 * @return Arrowpress_Import_REST_Response
	 * @throws Exception
	 * @since 1.10.1
	 *
	 * @see   step_plugins
	...
	private function _step_by_step( $step = '' ) {
		do_action( 'arrowpress_core_importer_start_step', $step );

		$callback_function = 'step_' . $step;

		$callable = apply_filters( "arrowpress_core_importer_step_$step", array( $this, $callback_function ) );

		if ( is_callable( $callable ) ) {
			return call_user_func( $callable );
		}

		throw Arrowpress_Error::create( __( 'Not found function' . $callable, 'arrowpress-core' ), 3 );
	}
	...
	public function step_plugins() {
		...
		try {
			...
			if ( ! ( $count_all_plugins ) || $index_plugin >= $count_all_plugins ) {
				...
				$response->message       = esc_html__( 'Finish step activate plugins', 'arrowpress-core' );
			...
			$plugin = Arrowpress_Plugins_Manager::get_plugin_by_slug( $plugin_slug );
			...
			if ( $plugin ) {
				...
				if ( 'not_installed' === $status ) {
					...
					if ( $install ) {
						...
						throw new Exception(
							sprintf(
								__( 'Installing %1$s plugin failed. %2$s', 'arrowpress-core' ),
								$plugin->get_name(),
								$string
							)
						);
					...
					if ( $result_active ) {
						...
						throw new Exception(
							sprintf(
								__( 'Activate %1$s plugin failed. %2$s', 'arrowpress-core' ),
								$plugin->get_name(),
								$string
							)
						);
	...
	public function step_download_demo_data() {
		...
		try {
			...
			do_action( 'arrowpress_core_importer_next_step', 'plugins', '', '' );

			$response->step_finished = 1;
			$response->status        = 'success';
			$response->percentage    = 100;
			$response->message       = esc_html__( 'Download demo data successfully', 'arrowpress-core' );
		}
	...
	public function step_theme_options() {
		...
		try {
			...
			$result       = Arrowpress_Importer_Service::theme_options( $setting_file );

			$response->status        = 'success';
			$response->message       = 'Set theme options success';
			$response->percentage    = 100;
			$response->step_finished = 1;
			// fix remove page of woo when install plugin
			if ( $response->step_finished == 1 ) {
				do_action( 'arrowpress_core_before_importer_main_content' );
			}
		}
		catch ( Arrowpress_Error $exception ) {
			$response->message = $exception->getMessage();
	...
	public function step_main_content() {
		...
		try {
			$prepare_wp_import = get_option( 'arrowpress_importer_prepare_wp_import', false );

			if ( ! $prepare_wp_import ) {

				$this->_prepare_demo_content();

				update_option( 'arrowpress_importer_prepare_wp_import', true );
			...
			$arrowpress_wp_import = Arrowpress_WP_Import_Service::instance();
 			$result               = $arrowpress_wp_import->import_posts( $fetch_attachment );
			 ...
			 */
			$arrowpress_wp_import->backfill_parents();
			$arrowpress_wp_import->backfill_attachment_urls();
			$arrowpress_wp_import->remap_featured_images();
	...
	public function step_media() {
		$response             = new Arrowpress_Import_REST_Response();
		$arrowpress_wp_import = Arrowpress_WP_Import_Service::instance();
		$result               = $arrowpress_wp_import->import_posts( true );
		...
		$response->status        = 'success';
		$response->message       = esc_html__( 'Import media success', 'arrowpress-core' );
		$response->percentage    = 100;
		$response->step_finished = 1;
	...
	public function step_widgets() {
		...
		try {
            ...
            $dir = $this->_get_dir_current_demo();
			$current_demo = self::get_current_demo_data();
			// no import file default content in theme
			if ( isset( $current_demo['default_content'] ) && ($current_demo['default_content'] == true )  ) {
                if(file_exists(  Arrowpress_Importer::get_base_dir() . '/widget/widget_data.json' )){
                    $dir = Arrowpress_Importer::get_base_dir();
                }
  			}

			$widget_file  = $dir . '/widget/widget_data.json';
 			$widget_logic = $dir . '/widget/widget_logic_options.txt';
 			Arrowpress_Importer_Service::widget( $widget_file, $widget_logic );
	...
	public function step_revslider() {
		...
		try {
			...
			Arrowpress_Importer_Service::revslider( $revsliders );
	...
	private function _try_step( $ext = null ) {
		...
		do_action( 'arrowpress_core_importer_try_step', $current, $ext );
	...
	private function _send_response( $data ) {
		echo '<!-- ARROWPRESS_IMPORT_START -->';
		header( 'Content-Type: application/json; charset=' . get_option( 'blog_charset' ) );
		echo wp_json_encode( $data );
		echo '<!-- ARROWPRESS_IMPORT_END -->';
