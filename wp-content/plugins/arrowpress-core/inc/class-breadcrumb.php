<?php
/**
 * Arrowpress_Breadcrumbs class.
 *
 */

namespace ArrowPressCoreBreadcrumbs;

if ( ! class_exists( 'Arrowpress_Breadcrumbs' ) ) {
	class Arrowpress_Breadcrumbs {
		/**
		 * @var array
		 */
		public $data = array();
		...
		public function breadcrumb_structured_data( $crumbs ) {
			...
			$this->data[] = apply_filters( 'arrowpress/structured_data/breadcrumblist', $markup, $crumbs );
		}
		...
		public function get_structured_data() {
			...
			$types = apply_filters( 'arrowpress/structured_data/types', array( 'breadcrumblist' ) );
		 ...
		 * @return array
		 */
		public function arrowpress_get_breadcrumb() {
			return apply_filters( 'arrowpress_get_breadcrumb', $this->crumbs, $this );
		}
		...
		public function render() {
			...
			$text_home = apply_filters( 'arrowpress_breadcrumb_text_home', _x( 'Home', 'breadcrumb', 'arrowpress-core' ) );
			...
			if ( ! is_front_page() || is_paged() ) {
				...
				// Structured
				$this->breadcrumb_structured_data( $this->arrowpress_get_breadcrumb() );

				return $this->arrowpress_get_breadcrumb();
			}
		...
		protected function add_crumbs_404() {
			$this->add_crumb( __( 'Error 404', 'arrowpress-core' ) );
		}
		...
		protected function add_crumbs_search() {
			if ( is_search() ) {
				$this->add_crumb( sprintf( esc_html__( 'Search results for &ldquo;%s&rdquo;', 'arrowpress-core' ), get_search_query() ), remove_query_arg( 'paged' ) );
			}
		}
		...
		protected function add_crumbs_current_archive_single( $post_id = '' ) {
			if ( get_post_type() == 'lp_course' ) {
				...
				$taxonomy = apply_filters( 'arrowpress_breadcrumb_taxonomy_single_post', '' );
			}
			...
			if ( $terms ) {
				$main_term = apply_filters( 'arrowpress_breadcrumb_main_term', $terms[0], $terms );
		...
		protected function add_crumbs_post_type_archive() {
			...
			if ( $post_type ) {
				if ( get_post_type() == 'lp_course' ) {
					$custom_name = get_option( 'learn_press_courses_page_id' ) ? get_the_title( get_option( 'learn_press_courses_page_id' ) ) : '';
				} elseif ( get_post_type() == 'product' ) {
					$custom_name = get_option( 'woocommerce_shop_page_id' ) ? get_the_title( get_option( 'woocommerce_shop_page_id' ) ) : '';
				} else {
					$custom_name = apply_filters( 'arrowpress_breadcrumb_post_type_name', '' );
				}

				$this->add_crumb( $custom_name ? $custom_name : $post_type->labels->name, get_post_type_archive_link( get_post_type() ) );
			}
		}
		...
		protected function add_crumbs_author() {
			...
			$this->add_crumb( sprintf( __( 'Author: %s', 'arrowpress-core' ), $userdata->display_name ) );
		}
		...
		protected function crumb_paged() {
			if ( get_query_var( 'paged' ) ) {
				$this->add_crumb( sprintf( esc_html__( 'Page %d', 'arrowpress-core' ), get_query_var( 'paged' ) ) );
			}
		}
	}
}
