...
/**
 * Class Arrowpress_Notification.
 *
 * @since 0.8.7
 */
if ( ! class_exists( 'Arrowpress_Notification' ) ) {

	class Arrowpress_Notification extends Arrowpress_Singleton {
		/**
		 * @since 0.8.7
		 *
		 * @var string
		 */
		private static $key_options = 'arrowpress_hide_notifications';
		...
		/**
		 * Arrowpress_Notification constructor.
		 *
		 * @since 0.8.7
		 */
		...
		private function init_hooks() {
			add_action( 'arrowpress_dashboard_notifications', array( __CLASS__, 'print_notifications' ) );
			add_action( 'admin_notices', array( __CLASS__, 'print_global_notifications' ) );
			add_action( 'admin_enqueue_scripts', array( $this, 'enqueue_scripts' ) );
			add_action( 'wp_ajax_arrowpress_hide_notification', array( $this, 'ajax_hide_notification' ) );
		}
		...
		public function enqueue_scripts() {
			wp_enqueue_script( 'arrowpress-notifications', ARROWPRESS_CORE_ADMIN_URI . '/assets/js/notifications.js', array( 'wp-util', 'jquery', 'backbone', 'underscore' ), ARROWPRESS_CORE_VERSION );
		...
		private function localize_scripts() {
			wp_localize_script( 'arrowpress-notifications', 'arrowpress_notifications', array(
				'ajax' => admin_url( 'admin-ajax.php?action=arrowpress_hide_notification' ),
			) );
		}
		...
		public static function print_global_notifications() {
			...
			$notifications = $arrowpress_notifications;
			...
			foreach ( $notifications as $notification ) {
                ...
                <div class="tc-notice <?php echo esc_attr( $class ); ?>" data-id="<?php echo esc_attr( $notification['id'] ); ?>">
					...
					<?php if ( $notification['dismissible'] ): ?>
                        <button type="button" class="arrowcore-notice-dismiss button button-secondary" style="margin: 0 0 0.8em 10px;"><span>Dismiss this notice.</span></button>
					<?php endif; ?>
                </div>
				<?php
			}
		...
		public static function add_notification( $notification ) {
			...
			$arrowpress_notifications[] = $notification;
		}
	}
}
