	...
	public function import( $request ) {
		...
		try {
			...
			if ( class_exists( '\Arrowpress_Product_Registration' ) ) {
				...
				$body['purchase_token'] = ! empty( $token ) ? $token : '';
			}
			...
			if ( isset( $api_body['code'] ) && $api_body['code'] === 'no_code' ) {
				throw new \Exception( class_exists( '\Arrowpress_Product_Registration' ) ? 'Please <a href="' . admin_url( '/admin.php?page=arrow-dashboard' ) . '" target="_blank" rel="noopener">Active theme</a> to continue' : 'Please register your site to use this feature.' );
			}

					// go to thim license page.
			if ( $api_body['code'] === 'no_purchase_code' ) {
				throw new \Exception( class_exists( '\Arrowpress_Product_Registration' ) ? 'Please <a href="' . admin_url( '/admin.php?page=arrow-license' ) . '" target="_blank" rel="noopener">add purchase code</a> to continue.' : 'Please register your site to use this feature.' );
			}

			if ( $api_body['code'] === 'invalid_purchase_code' ) {
				throw new \Exception( class_exists( '\Arrowpress_Product_Registration' ) ? 'Please <a href="' . admin_url( '/admin.php?page=arrow-license' ) . '" target="_blank" rel="noopener">add valid purchase code</a> to continue.' : 'Please register your site to use this feature.' );
			}
