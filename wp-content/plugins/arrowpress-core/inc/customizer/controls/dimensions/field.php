...
class Dimensions extends Field {
	...
	public $type = 'arrowpress-dimensions';
	...
	public function init( $args = array() ) {
		...
		new \ArrowTheme\Customizer\Field\Generic(
			wp_parse_args(
				array(
					...
					'choices'           => array(
						'type'        => 'hidden',
						'parent_type' => 'arrowpress-dimensions',
					),
				),
				$args
			)
		...
		foreach ( $args['default'] as $choice => $default ) {
			...
			$wrapper_attrs = array(
				'data-arrowpress-parent-control-type'    => 'arrowpress-dimensions',
				'data-arrowpress-parent-control-setting' => $args['id'],
				'class'                            => isset( $args['wrapper_attrs']['class'] ) ? $args['wrapper_attrs']['class'] . ' arrowpress-group-item arrowpress-w' . $width : '{default_class} arrowpress-group-item arrowpress-w' . $width,
			);

			if ( $item_count === 1 ) {
				$wrapper_attrs['class'] .= ' arrowpress-group-start';
			}

			if ( in_array( $item_count, $break_indexes, true ) ) {
				$wrapper_attrs['class'] .= ' arrowpress-group-break';
			}

			if ( $item_count === $total_items ) {
				$wrapper_attrs['class'] .= ' arrowpress-group-end';
			}

			new \ArrowTheme\Customizer\Field\Dimension(
				wp_parse_args(
					array(
						'type'           => 'arrowpress-dimension',
						'id'             => $args['id'] . '[' . $choice . ']',
