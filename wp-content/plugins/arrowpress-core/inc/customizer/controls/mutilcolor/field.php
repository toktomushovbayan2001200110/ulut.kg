...
class Multicolor extends Field {
	...
	public $type = 'arrowpress-multicolor';
	...
	public function init( $args ) {
		...
		$parent_control_args = wp_parse_args(
			array(
				...
				'choices'           => array(
					'type'        => 'hidden',
					'parent_type' => 'arrowpress-multicolor',
				),
		...
		new \ArrowTheme\Customizer\Field\Generic( $parent_control_args );
		...
		foreach ( $args['choices'] as $choice => $choice_label ) {
			...
			$classnames  = isset( $args['wrapper_attrs']['class'] ) ? $args['wrapper_attrs']['class'] . ' arrowpress-group-item' : '{default_class} arrowpress-group-item';
			$classnames .= 1 === $loop_index ? ' arrowpress-group-start' : ( $loop_index === $total_colors ? ' arrowpress-group-end' : $classnames );
			...
			$control_args = wp_parse_args(
				array(
					...
					'wrapper_attrs'  => array(
						'data-arrowpress-parent-control-type' => 'arrowpress-multicolor',
						'class'                         => $classnames,
					),
			...
			new \ArrowTheme\Customizer\Field\Color( $control_args );
		}
	}
	...
	public static function sanitize( $value ) {
		foreach ( $value as $key => $subvalue ) {
			$value[ $key ] = \ArrowTheme\Customizer\Field\Color::sanitize( $subvalue );
		}
	...
	public function output_control_classnames( $classnames ) {
		$classnames['arrowpress-multicolor'] = '\ArrowTheme\Customizer\CSS\Multicolor';
