...
use ArrowTheme\Customizer\Utils\Helper;
use ArrowTheme\Customizer\Modules\CSS\Generator;
...
class Css {	
	...
	private static $css_handle = 'arrowpress-customizer-styles';

	private static $inline_styles_id = 'arrowpress-customizer-inline-styles';

	public function __construct() {
		add_action( 'arrowpress_customizer_field_init', array( $this, 'field_init' ), 10, 2 );
		add_action( 'init', array( $this, 'init' ) );
	}

	public function init() {
		if ( ! apply_filters( 'arrowpress_customizer_output_inline_styles', true ) ) {
	...
	public function print_styles_inline() {
		...
		if ( defined( 'ARROWPRESS_CUSTOMIZER_NO_OUTPUT' ) && true === ARROWPRESS_CUSTOMIZER_NO_OUTPUT ) {
			$should_print = false;
		}
		...
		$inline_styles_id = apply_filters( 'arrowpress_customizer_inline_styles_id', self::$inline_styles_id );
	...
	public function enqueue_styles() {
		$args = array(
			'action' => apply_filters( 'arrowpress_customizer_styles_action_handle', self::$css_handle ),
		);
	...
	public function print_styles() {
		...
		do_action( 'arrowpress_customizer_dynamic_css' );
	}
